...
definition(
    name: "Socket Power Is Out edited for restoration",
    namespace: "wackware",
...
def installed()
{
    ...
    subscribe(myDevice, "powerSource.powered", onPoweredPowerHandler)
}
...
def onPoweredPowerHandler(evt) {
	log.trace "$evt.value: $evt, $settings"
	def msg = "${myDevice.label ?: myDevice.name} detected going to house power"
