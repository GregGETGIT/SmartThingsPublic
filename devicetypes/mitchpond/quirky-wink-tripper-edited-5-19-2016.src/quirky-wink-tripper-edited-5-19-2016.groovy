...
metadata {
	definition (name: "Quirky/Wink Tripper Edited 5/19/2016", namespace: "mitchpond", author: "Mitch Pond") {
...
def poll() {
    ...
    def maxReportingIntervalSeconds = (6 * 60 * 60) // 6 Hours
    if (!state.lastBatteryReport || (now - state.lastBatteryReport) > maxReportingIntervalSeconds) {
        [
            "zcl global send-me-a-report 1 0x20 0x20 3600 $maxReportingIntervalSeconds {01}", "delay 200", 
